...
- [`Calf.Types.BigO`](./src/Calf/Types/BoundedFunction.agda) gives a definition of "big-O" asymptic bounds as a relaxation of `IsBounded`.
  In particular, an element of the type `given A measured-via size , f ∈𝓞(g)` (mixfix notation for `given_measured-via_,_∈𝓞_ A size f g`) is a number `n'` and a constant `k` along with a proof `h` that for all inputs `x` with `n' ≤ size x`, `f x` is bounded by `k` multiples of `g (size x)`, denoted `n' ≤n⇒f[n]≤ k g[n]via h` (mixfix notation for `_≤n⇒f[n]≤_g[n]via_ n' k h`).
